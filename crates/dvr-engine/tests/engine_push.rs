...
fn git_push_appears_only_in_push_rs() {
    ...
    let allowlist = [
        (
            "crates/dvr-daemon/src/state.rs",
            "Command::new(\"git\").args([\"config\", key])",
        ),
        (
            "crates/dvr-engine/src/push.rs",
    ...
    assert!(seen.contains("crates/dvr-engine/src/push.rs"), "guardrail grep went vacuous");
}
