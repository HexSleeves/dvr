...
impl ApiFailure {
    ...
    fn from_engine(err: anyhow::Error) -> Self {
        match err.downcast_ref::<dvr_engine::EngineError>() {
            Some(dvr_engine::EngineError::NotFound(_)) => {
                Self::new(StatusCode::NOT_FOUND, ErrorCode::NotFound, format!("{err:#}"))
            }
            Some(dvr_engine::EngineError::Conflict(_)) => {
                Self::new(StatusCode::CONFLICT, ErrorCode::Conflict, format!("{err:#}"))
            }
            Some(dvr_engine::EngineError::Guardrail(_)) => Self::new(
                StatusCode::FORBIDDEN,
                ErrorCode::GuardrailRefused,
                format!("{err:#}"),
            ),
            Some(dvr_engine::EngineError::Invalid(_)) => Self::new(
...
mod tests {
    ...
    fn conflict_engine_error_maps_to_http_conflict() {
        let failure = ApiFailure::from_engine(
            dvr_engine::EngineError::Conflict("path is conflicted: src/lib.rs".into()).into(),
        );
