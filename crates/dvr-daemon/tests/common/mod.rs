...
pub fn fresh_state_dir() -> TempDirGuard {
    ...
    let dir = PathBuf::from(format!("/tmp/dvr-{}-{n}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).unwrap();
    TempDirGuard(dir)
}

/// Spawns `dvr_daemon::run_with_dir` on a fresh state dir and waits until
...
pub async fn spawn_daemon_in(dir: &Path) -> TestDaemon {
    let socket = dir.join("dvrd.sock");
    let run_dir = dir.to_path_buf();
    let task = tokio::spawn(async move {
        if let Err(err) = dvr_daemon::run_with_dir(run_dir).await {
...
/// leftovers). Duplicated from dvr-engine's test fixture — test helpers aren't
