...
use dvr_engine::RepoEngine;
use dvr_proto::{RepoId, RepoInfo};
...
impl DaemonState {
    ...
    pub async fn load(dir: &Path) -> anyhow::Result<Self> {
        ...
        if registry_path.exists() {
            ...
            for info in infos {
                ...
                let settings = dvr_engine::settings::make_settings(&identity.0, &identity.1)?;
    ...
    pub async fn register(&self, path: &Path) -> Result<RepoInfo, RegisterError> {
        ...
        let settings = dvr_engine::settings::make_settings(&self.inner.identity.0, &self.inner.identity.1)
...
fn git_identity() -> (String, String) {
    ...
    (
        get("user.name").unwrap_or_else(|| "dvr".to_string()),
        get("user.email").unwrap_or_else(|| "dvr@localhost".to_string()),
    )
}
