//! Daemon singleton: two bgd's on the same state dir would both rebind the
...
async fn second_daemon_on_same_state_dir_fails_fast() {
    ...
    let second = tokio::time::timeout(Duration::from_secs(5), bg_daemon::run_with_dir(dir)).await;
    let err = second
