//! Unix-socket HTTP client for the bgd daemon, with transparent auto-start:
//! if nothing answers on `socket_path()`, spawn the `bgd` binary sitting next
...
impl Client {
    /// Connects to the daemon on `socket_path()`, auto-starting `bgd` (the
    /// sibling of the current executable) if nothing is listening yet.
    pub async fn connect() -> anyhow::Result<Client> {
        let client =
            Client { socket: bg_daemon::socket_path(), http: HyperClient::unix() };
        ...
        loop {
            ...
            anyhow::ensure!(
                ...
                bg_daemon::state_dir().join("bgd.log").display()
            );
            tokio::time::sleep(Duration::from_millis(100)).await;
        }
    }

    /// Spawns `bgd` detached, with stdout/stderr appended to
    /// `state_dir()/bgd.log` so daemon noise never lands on the CLI terminal.
    fn spawn_daemon(&self) -> anyhow::Result<()> {
        ...
        let bgd = exe
            ...
            .join("bgd");
        let state = bg_daemon::state_dir();
        std::fs::create_dir_all(&state)
            .with_context(|| format!("cannot create state dir {}", state.display()))?;
        let log = std::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open(state.join("bgd.log"))?;
        let err_log = log.try_clone()?;
        std::process::Command::new(&bgd)
            .stdin(Stdio::null())
            .stdout(Stdio::from(log))
            .stderr(Stdio::from(err_log))
            .spawn()
            .with_context(|| format!("failed to start daemon binary {}", bgd.display()))?;
        Ok(())
    }
...
fn api_failure(status: StatusCode, bytes: &[u8]) -> ApiFailure {
    match serde_json::from_slice::<bg_proto::ApiError>(bytes) {
