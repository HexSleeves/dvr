//! `dvr` — thin client over the dvrd daemon's unix-socket HTTP API.
...
#[derive(Parser)]
#[command(name = "dvr", version, about = "jj-powered repo daemon client")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
#[derive(Subcommand)]
enum DaemonCmd {
    /// Run the daemon in the foreground (same as the `dvrd` binary)
    Run,
}
